...
mydata <- pisa.select.merge(
    ...
    student = c(
        ...
        #X3.3 SCHOOL-RELATED USE AT HOME
    ...
    school = c(
        ...
        "SC156Q06HA" #pecfc polcy abt using Social Networks (<Facebook>) in teaching and learning
    ),
...
library(data.table)
setDTthreads(0)
fwrite(mydata, file = "ict1.dat", sep = "\t", row.names = F, col.names = T)
